...
function App() {
  ...
  useInput((input, key) => {
    ...
    if (phase === "login") {
      ...
      if (key.tab || key.downArrow || input === "j") {
        const order: Array<typeof loginFocus> = loginUseEnv ? ["totp", "submit"] : ["email", "password", "totp", "submit"];
    ...
    if (phase === "inventory") {
      ...
      if (key.downArrow || input === "j") {
        ...
        const idx = Math.max(0, flat.findIndex((r) => r.uuid === selectedUuid));
      ...
      <Box key={uuid} flexDirection="column">
        ...
        {expanded ? (
          <Box flexDirection="column" paddingLeft={2}>
            ...
            <Text dimColor>
              actions: {h?.host ? "o=on f=off d=diagnostics" : "s=scan LAN to find IP"}
            </Text>
          </Box>
        ) : null}
      </Box>
  ...
  return (
    <Box flexDirection="column" padding={1} gap={1}>
      <Text bold>{title}</Text>
      <Text dimColor>q/Esc: quit · j/k or arrows: move · Enter: open · r: refresh · s: scan · o/on f/off · d: diagnostics</Text>
      <Text dimColor>Config: {cfgPath}</Text>
      {error ? <Text color="red">{error}</Text> : null}
      {busy ? <Text dimColor>Busy: {busy}</Text> : null}
